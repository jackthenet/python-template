...
#### T-011 — S4.4 Refactor (keep GREEN) — GREEN MAINTAINED

- **Task:** T-011 "filemanagement enforcement wiring (principal param + @requires_permission + permission_service constructor + feature_actions)" — REQ-024 / AC-029 (per-feature test gate).
- **Refactor assessment:** the T-011 wiring is well-structured and consistent with the T-008/T-009/T-010 pattern (module-level `_SYSTEM_PRINCIPAL = Principal()` singleton for B008-clean defaults, trailing `principal: Principal = _SYSTEM_PRINCIPAL` param, optional `permission_service: PermissionChecker | None = None` constructor param saved to `self._permission_service`, ADR-070 boundary respected — the feature imports only `backend.shared` and the `PermissionCatalog` annotation is `TYPE_CHECKING`-only). No duplication, complexity, naming, or boundary issues.
- **Change made (clarity, behavior-neutral):** the spec's D13 table lists **all 10** filemanagement methods as enforced (no exempt column), but the code decorates only **8** — `download` and `list_files` carry the `principal` param but no `@requires_permission`, a deviation mandated by the **AC-029 test contract** (the deny section calls `list_files` and expects `[]`; the allow section expects `download` to record no check), not by the spec. Added a 3-line `# ADR-071 / AC-029:` comment above each of those two methods documenting the intentional principal-only (non-enforced) wiring so a reader comparing code to spec does not misread the missing decorators as a bug. No code/logic change.
- **GREEN (targeted — the task's 1 test, from the DAG):** `uv run pytest tests/acceptance/permissions/test_enforcement.py::test_enforced_method_denies_without_permission -q` → **1 passed, 0 failed** after the refactor (GREEN maintained).
- **Ruff (T-011 changed path):** `uv run ruff check src/backend/filemanagement/service.py src/backend/filemanagement/feature_actions.py` → **All checks passed!**; `uv run ruff format --diff` on `service.py` → the only reformats are the 2 pre-existing `_validate_content` calls (the `key, namespace, content,` / `key, "avatars", content,` lines) already recorded as pre-existing/out-of-scope in S4.2 — the new comment lines are format-clean (line-length 120 respected; none of the new lines appear in the format diff).
- **Next:** S4.5 (T-011) — commit + update status.

#### T-012 — S4.1 Pick task + confirm RED — RED CONFIRMED
...
- **Ready:** confirmed — `dependencies: ['T-001', 'T-003']`; all **VERIFIED** in `docs/tasks/user-roles-permissions.tasks.json` and `.github/task-runner/tasks.json` (T-001 shared enforcement plumbing; T-003 permissions foundation). T-012 is `PENDING` → ready.
- **RED command (targeted — the task's 1 test, from the DAG):** `uv run pytest tests/acceptance/mail/test_enforcement_wiring.py::test_mail_enforcement_wiring -v` (the DAG's `red_command` is already in the real file path form — no `::`-in-directory issue).
- **Result:** **1 failed, 0 passed, 0 collection errors** — RED confirmed before implementation (the test collected cleanly; the failure is in the **test body (call phase)** — none in setup/fixture/collection/import).
- **Per-test outcome:**

| Test | Result | Meaning |
|---|---|---|
| `tests/acceptance/mail/test_enforcement_wiring.py::test_mail_enforcement_wiring` (REQ-024 / ADR-071) | **FAILED** (behavior) | the T-012 mail enforcement wiring is not implemented — the expected RED signal |
- **Failure mode of the RED test** (the established RED pattern for this change — the missing wiring, in the test body, not a setup/collection error):
...
#### T-012 — S4.4 Refactor (keep GREEN) — NO CHANGES (already well-structured)

- **Refactor assessment:** the T-012 source (`src/backend/mail/service.py`, `src/backend/mail/feature_actions.py`, `src/backend/mail/__init__.py`) was reviewed for structural improvements (duplication, complexity, naming, boundaries). **No meaningful improvement is possible — no changes were made** (per the S4.4 done criteria: record that and make no changes). No observable behavior change is possible (zero file changes); no test modifications.
- **Candidates considered (all rejected):**
  - **Helper deriving the permission key from the method name** (e.g., an `@_enforce` replacing `@requires_permission("mail.<method>")`): the permission keys are a **stable, externally observable contract** — declared in `feature_actions.register_actions` (D3), enforced by the permissions catalog, and enumerated in the spec Section 3 initial catalog table. Deriving them from method names would couple the stable contract to method naming (a method rename would silently change the permission key = an observable behavior change). The explicit form is exactly what ADR-071 prescribes and what the shared pattern across all six features uses; the dedup gained is only the `mail.` prefix — negligible. (Same rejection as T-008/T-009/T-010 S4.4.)
  - **Extracting the trailing `principal` parameter:** not possible — ADR-071 requires the parameter in each method's public signature (backward compatibility: existing positional call sites are unaffected), and the shared decorator resolves it via `inspect.signature`.
  - **Extracting the 3 description strings in `feature_actions.py`:** the dict is the catalog declaration (D3) and the single source of truth for the human-readable descriptions; moving it to a separate module adds an import with zero structural gain, and the key → description mapping must stay with the `register_feature("mail", ...)` call. (Same rejection as T-010 S4.4.)
  - **Forwarding `principal` in the two delegating methods** (`send_password_reset_email` / `send_email_verification_email` call `self.send_email(...)` without the caller's `principal`, so the inner `mail.send_email` check evaluates as the system principal): forwarding would be an **observable behavior change**, not a behavior-neutral refactor — a caller holding only `mail.send_password_reset_email` (but not `mail.send_email`) could currently send reset emails (the bootstrap system set includes `mail.send_email`, spec D10), and forwarding would deny them at the inner check. The current behavior is consistent with the spec's independent-action model (the 3 mail actions are separate catalog actions, all in the bootstrap set) and is pinned by neither the test contract nor the spec's delegation semantics. Out of S4.4 scope (no behavior change); no change made.
- **Consistency check (read-only, no changes):** the 3 decorator keys in `service.py` (`mail.send_email`, `mail.send_password_reset_email`, `mail.send_email_verification_email`) match 1:1 the 3 keys declared in `feature_actions.register_actions`; all 3 public `MailService` methods carry the trailing `principal: Principal = _SYSTEM_PRINCIPAL` parameter (the mail exempt set is empty — all 3 are enforced; the private members `_get_transport` / `_publish` take no parameters); `__init__.py` exports `register_actions` in both the import list and `__all__` (alphabetical order preserved).
...
#### T-013 — S4.2 Implement + confirm GREEN — GREEN CONFIRMED

- **Task:** T-013 "sessionmanagement enforcement wiring (principal param + @requires_permission + permission_service constructor + feature_actions)" — REQ-024 (no AC — per-feature test gate).
- **Implementation (per DAG task T-013, ADR-071/ADR-070/B008/AC-031 — same pattern as T-008/T-009/T-010/T-011/T-012):**
  - `src/backend/sessionmanagement/service.py`: imported `PermissionChecker`, `Principal`, `requires_permission` from `backend.shared`; added the module-level `_SYSTEM_PRINCIPAL = Principal()` singleton (ADR-071 / B008 — the default trailing principal is the system principal, EDGE-022); added the optional `permission_service: PermissionChecker | None = None` constructor parameter (saved to `self._permission_service`; `None` = standalone mode with no enforcement, AC-031); added the trailing `principal: Principal = _SYSTEM_PRINCIPAL` parameter to **all 6 public `SessionService` methods** (`list_sessions`, `revoke_session`, `logout_all_sessions`, `logout_other_sessions`, `revoke_all_sessions`, `cleanup_expired`); applied `@requires_permission("sessionmanagement.<method>")` to each (permission keys `sessionmanagement.list_sessions`, `sessionmanagement.revoke_session`, `sessionmanagement.logout_all_sessions`, `sessionmanagement.logout_other_sessions`, `sessionmanagement.revoke_all_sessions`, `sessionmanagement.cleanup_expired` — the sessionmanagement exempt set is empty, spec Section 3). The session repository (`get_by_token_hash` over SHA-256 hashes) remains the real `SessionLookup` implementation the check uses for session validation (REQ-017).
  - `src/backend/sessionmanagement/feature_actions.py` (new): feature-owned `register_actions(catalog)` declaring the 6 `sessionmanagement.<method>` actions via `catalog.register_feature("sessionmanagement", [...])`; the `PermissionCatalog` import is `TYPE_CHECKING` only (ADR-070 — the feature never imports `backend.permissions` at runtime).
  - `src/backend/sessionmanagement/__init__.py`: exports `register_actions` in the public API + `__all__` (same as the T-008/T-010/T-012 `__init__` pattern).
- **GREEN command (targeted — the task's 1 test, from the DAG):** `uv run pytest tests/acceptance/sessionmanagement/test_enforcement_wiring.py::test_sessionmanagement_enforcement_wiring -v`.
- **Result:** **1 passed, 0 failed, 0 collection errors** — GREEN confirmed after implementation.
- **Per-test outcome:**

| Test | Result | Meaning |
|---|---|---|
| `tests/acceptance/sessionmanagement/test_enforcement_wiring.py::test_sessionmanagement_enforcement_wiring` (REQ-024 / ADR-071) | **PASSED** | the T-013 sessionmanagement enforcement wiring is implemented — the ADR-071 contract is satisfied: all 6 public methods take the trailing `principal` (default = the system principal, `user_id=None`/`session_token=None`); the deny checker's denial propagates to the caller with the `sessionmanagement.<method>` keys recorded in order; the allow path proceeds evaluating the default principal as the system principal (EDGE-022); an explicit `Principal(user_id, session_token)` reaches the check; the constructor's `permission_service` defaults to `None` (standalone, AC-031); `feature_actions.register_actions` declares the 6 sessionmanagement catalog actions |
- **Regression check (the enforcement wiring must not break existing sessionmanagement tests):** `uv run pytest tests/acceptance/sessionmanagement/ tests/unit/sessionmanagement/ tests/integration/sessionmanagement/ tests/contract/sessionmanagement/ tests/property/sessionmanagement/ -v` → **63 passed, 6 failed** with the change (includes the targeted T-013 test). The 6 failures are **pre-existing and unrelated to T-013** — the T-002 pre-amendment-API issue: the shared helper `tests/authentication_test_helpers.py::create_user` still calls `UserCreate(..., role=...)` (singular `role`; `roles` is now required), failing with `ValidationError: 'roles' Field required` before any `SessionService` call; verified pre-existing: `git stash` → re-run → the same 6 failures → `git stash pop`. No T-013 regression (no previously-GREEN sessionmanagement test regressed).
- **Ruff (T-013 paths):** `uv run ruff check src/backend/sessionmanagement/service.py src/backend/sessionmanagement/feature_actions.py src/backend/sessionmanagement/__init__.py` → **All checks passed!**; `uv run ruff format --check` on the same paths → `feature_actions.py` + `__init__.py` already formatted; `service.py` has one **pre-existing** format deviation (the `valid = [...]` block in the untouched `_on_login_succeeded` handler — line 141 with the T-013 additions, line 121 on the clean file; verified pre-existing via `git stash`; the lines added in T-013 are format-clean; reformatting the pre-existing block is out of T-013 scope).
- **Next:** S4.3 (T-013) — Ruff gate on the changed paths.

#### T-013 — S4.4 Refactor (keep GREEN) — NO CHANGES (already well-structured), GREEN MAINTAINED

- **Task:** T-013 "sessionmanagement enforcement wiring (principal param + @requires_permission + permission_service constructor + feature_actions)" — REQ-024 (no AC — per-feature test gate).
- **Refactor assessment (no meaningful improvement possible — recorded, no changes made; same outcome class as T-012 S4.4):**
  - The 6 enforced public methods follow the exact ADR-071 house pattern — trailing `principal: Principal = _SYSTEM_PRINCIPAL` parameter + `@requires_permission("sessionmanagement.<method>")` — identical in shape to the sibling enforcement wirings (T-008/T-009/T-010/T-011/T-012); the permission keys are the stable `sessionmanagement.<method>` contract and are deliberately explicit per method (a key-derivation helper would hide that contract — rejected, cf. T-012 S4.4).
  - The module-level `_SYSTEM_PRINCIPAL = Principal()` singleton (B008 / ADR-071 / EDGE-022) is the documented lint-clean way to keep the argument defaults identical across methods; the optional `permission_service: PermissionChecker | None = None` constructor parameter is the minimal standalone-mode seam (AC-031) and is already documented in the module docstring.
  - `feature_actions.py` is minimal and clean: `register_actions(catalog)` declaring the 6 actions via `catalog.register_feature("sessionmanagement", [...])`, with the `PermissionCatalog` import under `TYPE_CHECKING` only (ADR-070 — no runtime `backend.permissions` import).
  - `__init__.py` exports `register_actions` in the public API + `__all__` per the house pattern (same as T-008/T-010/T-012).
  - Candidate rejected: unifying `list_sessions`' inline token resolution into `_resolve_token` — the inline path shares one `now` snapshot between the current-session validity check and the valid-only filter (documented INV-002 semantics); routing through `_resolve_token` (which computes its own `now`) would change that snapshot behavior, and the code is pre-existing (not a T-013 addition) — out of scope.
- **GREEN (re-run after the refactor step — zero file changes, so the S4.2/S4.3 GREEN still holds; re-run recorded as fresh post-refactor evidence):** `uv run pytest tests/acceptance/sessionmanagement/test_enforcement_wiring.py::test_sessionmanagement_enforcement_wiring -v` → **1 passed, 0 failed, 0 collection errors**.
- **Ruff (T-013 paths):** `uv run ruff check src/backend/sessionmanagement/service.py src/backend/sessionmanagement/feature_actions.py src/backend/sessionmanagement/__init__.py` → **All checks passed!**
- **No observable behavior changed** (zero source file changes); no test modification.
- **Next:** S4.5 (T-013) — Commit + update status (mark VERIFIED).

#### T-014 — S4.1 Pick task + confirm RED — RED NOT OBSERVED (tests already GREEN via dependency tasks)

- **Task:** T-014 "cross-cutting enforcement integration + composition root wiring (PermissionService as the shared PermissionChecker)" — REQ-005, AC-006, AC-030 (the final task in the DAG).
- **Ready:** confirmed — `dependencies: ['T-004', 'T-008', 'T-009', 'T-010', 'T-011', 'T-012', 'T-013']`; all **VERIFIED** in `docs/tasks/user-roles-permissions.tasks.json` and `.github/task-runner/tasks.json`. T-014 is `PENDING` → ready.
- **RED command (targeted — the task's 4 tests, from the DAG; run in real file-path form because the DAG's `red_command` uses `::` in the directory part, which pytest rejects):** `uv run pytest tests/acceptance/permissions/test_check_api.py::test_initial_catalog_exactly_60_keys tests/acceptance/permissions/test_enforcement.py::test_exempt_login_no_check tests/unit/permissions/test_edge_cases.py::test_default_principal_system tests/unit/permissions/test_edge_cases.py::test_login_before_permissions -v`
- **Result:** **4 passed, 0 failed, 0 collection errors** — **RED NOT observed before implementation** (the tests collect cleanly and pass in the test body / call phase).
- **Per-test outcome:**

| Test | Result | Meaning |
|---|---|---|
| `tests/acceptance/permissions/test_check_api.py::test_initial_catalog_exactly_60_keys` (AC-006) | **PASSED** | the six features' `feature_actions.register_actions` (implemented in T-008…T-013) already produce exactly the 60 catalog keys — the test builds the catalog itself; it does not exercise the composition root |
| `tests/acceptance/permissions/test_enforcement.py::test_exempt_login_no_check` (AC-030) | **PASSED** | self-contained: builds its own `PermissionService` + a wired `UserManager`/`AuthService` (the `permission_service` constructor parameter was added in T-008/T-009); the exempt-login behavior is already present |
| `tests/unit/permissions/test_edge_cases.py::test_default_principal_system` (EDGE-022) | **PASSED** | self-contained: builds its own `PermissionService` + a minimal enforced service using the shared `@requires_permission` decorator (T-001); the default-principal behavior is already present |
| `tests/unit/permissions/test_edge_cases.py::test_login_before_permissions` (EDGE-023) | **PASSED** | self-contained: builds its own `PermissionService` + a wired `UserManager`; the bootstrap-system-set behavior is already present |
- **Why RED is not observed (root cause):** all 4 T-014 tests are **self-contained** — each constructs its own service composition (`PermissionService` + the wired services) inside the test body and asserts the behavior. **None of them imports the composition root (`src/main.py`).** The behaviors they assert (exempt login, default principal, login-before-permissions, the 60-key catalog) were all made GREEN by the **dependency tasks** (T-004 check core; T-008…T-013 per-feature enforcement wiring, which added the `permission_service` constructor parameter, the trailing `principal` parameter, and the `feature_actions` modules). T-014's own remaining implementation — **wiring the composition root in `src/main.py`** (create the `PermissionService`, inject it as the shared `PermissionChecker` into all six services, call the six `register_actions` at startup) — is **not covered by any of the 4 tests** (verified: no test imports `main`/the composition root; `src/main.py` currently has no `PermissionService`/`register_actions` wiring).
- **Gate consequence:** the S4.1 RED gate is **NOT satisfied** — the task's tests do not FAIL before implementation, so a subsequent GREEN cannot be attributed to the T-014 implementation. This is a **test-coverage gap** for the composition-root wiring (the tests verify the behavior contract, not the wiring itself). Recorded for the orchestrator to decide (e.g., proceed to S4.2 with the composition-root wiring as the implementation and the 4 tests as the GREEN gate, or flag the coverage gap for a decision).
- **Next:** orchestrator decision required (RED gate not passed). If proceeding, S4.2 (T-014) — implement the composition root wiring in `src/main.py` and confirm the 4 tests stay GREEN.

#### T-014 — S4.2 Implement + confirm GREEN — GREEN (composition root wired; 4 tests pass; pre-existing failures unrelated)

- **Task:** T-014 "cross-cutting enforcement integration + composition root wiring (PermissionService as the shared PermissionChecker)" — REQ-005, AC-006, AC-030 (the final task in the DAG).
- **Orchestrator decision (S4.1 handoff):** proceed with option (a) — the composition-root wiring in `src/main.py` as the implementation, and the 4 tests as the GREEN gate (regression guard, not a RED gate).
- **Implementation (src/main.py — the composition root):**
  - Builds the static permission catalog (`_catalog`) from the six features' `register_actions` calls (REQ-004, REQ-005) — 60 keys.
  - Defines `_LazyPermissionService` and `_LazyUserManager` proxy classes to break the composition-root cycles (PermissionService ↔ UserManager, PermissionService ↔ SettingsRegistry).
  - Creates the shared `SettingsRegistry` (wired with the lazy PermissionService proxy) and sets it as the singleton.
  - Creates the shared `PermissionService` (with the SQLite repositories) and sets it on the lazy proxy.
  - Seeds the system-principal set with `BOOTSTRAP_SYSTEM_PERMISSIONS | {"settings.has", "settings.get_value"}` (the bootstrap set covers internal startup calls; the settings-read keys cover `setup_logger`'s live reads).
  - Creates the shared `UserManager` (wired with the shared PermissionService) and sets it on the lazy proxy.
  - Creates the remaining five services (AuthService, FileService, MailService, SessionService) each wired with the shared PermissionService.
  - Calls `setup_logger()` (REQ-011).
- **GREEN (targeted — the task's 4 tests):** `uv run pytest tests/acceptance/permissions/test_check_api.py::test_initial_catalog_exactly_60_keys tests/acceptance/permissions/test_enforcement.py::test_exempt_login_no_check tests/unit/permissions/test_edge_cases.py::test_default_principal_system tests/unit/permissions/test_edge_cases.py::test_login_before_permissions -v` → **4 passed, 0 failed, 0 collection errors**.
- **Permissions suite (regression guard — all 58 permissions tests):** `uv run pytest tests/acceptance/permissions/ tests/unit/permissions/ -v` → **58 passed, 0 failed**.
- **Full suite (pre-existing failures unrelated to T-014):** `uv run pytest tests/ -v` → **125 failed, 509 passed, 1 skipped**. The 125 failures are **pre-existing** (verified: the same tests fail with `src/main.py` stashed — root cause: the `UserCreate` model now requires `roles: list[str]` from the user-roles-permissions work, but the test helpers don't include it). The composition root wiring does not introduce new failures.
- **Ruff (T-014 paths):** `uv run ruff check src/main.py` → **All checks passed!**
- **Mypy (T-014 paths):** `uv run mypy src/main.py` → **Success: no issues found in 1 source file**.
- **Next:** S4.3 (T-014) — Ruff gate (already passed); S4.4 (T-014) — Refactor; S4.5 (T-014) — Commit + update status (mark VERIFIED).

#### T-014 — S4.3 Ruff — clean (no changes needed)

- **Ruff (T-014 paths):** `uv run ruff check src/main.py` → **All checks passed!** (re-confirmed; no code changes needed — S4.2 already left the file lint-clean).
- **Next:** S4.4 (T-014) — Refactor; S4.5 (T-014) — Commit + update status (mark VERIFIED).

#### T-014 — S4.4 Refactor (keep GREEN) — no changes (already well-structured) — GREEN maintained, ruff clean

- **Assessment (no behavior-preserving refactor found meaningful):**
  - **Linear composition root with section headers** — the wiring order (catalog → lazy proxies → SettingsRegistry → PermissionService → settings registration → UserManager → the five services → `setup_logger`) is the essence of the design (the two documented cycles broken with lazy proxies) and is documented inline. The module-level import-time wiring is a **contract**: `tests/acceptance/settings_coverage/test_wiring.py::test_main_wires_all_features` (AC-003, pre-existing) executes `import main` in a subprocess and asserts the import-time side effects (feature settings registered in the shared singleton). Wrapping the wiring in a helper function (the suggested candidate) would hide the order and the module-level names behind a function call with a ~100-line re-indentation diff and zero functional gain — the linear script is the most direct representation of the wiring order.
  - **The two proxy classes are intentionally distinct** — `_LazyPermissionService` explicitly exposes the structural `PermissionChecker` protocol methods (`require_permission`/`has_permission`), documenting the interface the six services depend on (not duplication); `_LazyUserManager` is a minimal `__getattr__` proxy. Merging them into one generic lazy proxy would reduce clarity; the class names document what is proxied.
  - **DB-path convention is internally consistent** — reused paths are named constants (`_PERMISSION_DB` ×3, `_AUTH_DB` ×4); the two inline SQLite strings are each used exactly once (consistent with the file's own convention; extracting single-use constants would be noise).
  - **Docstrings/section comments verified accurate** against the actual wiring order (e.g., "created (and set on the proxy) before the settings registration"; the four feature-settings registrations match the pre-existing set and the AC-003 test contract).
  - **Pre-existing state (out of scope for S4.4 — recorded as a finding):** `backend.mail`, `backend.filemanagement`, and `backend.sessionmanagement` expose `register_settings`, but the composition root does not call them — this is pre-existing (the pre-S4.2 `src/main.py` registered exactly the same four features: logging, authentication, usermanagement, eventbus). Adding the three calls would be a behavior change (new settings keys in the registry) and is NOT done in S4.4; flagged for Phase 5/orchestrator awareness.
- **GREEN maintained (targeted — the task's 4 tests, re-run for fresh evidence):** `uv run pytest tests/acceptance/permissions/test_check_api.py::test_initial_catalog_exactly_60_keys tests/acceptance/permissions/test_enforcement.py::test_exempt_login_no_check tests/unit/permissions/test_edge_cases.py::test_default_principal_system tests/unit/permissions/test_edge_cases.py::test_login_before_permissions -v` → **4 passed, 0 failed**.
- **AC-003 contract test (composition-root import-time wiring):** `uv run pytest tests/acceptance/settings_coverage/test_wiring.py -v` → **1 passed** (baseline, pre-refactor — confirms the import-time wiring contract holds).
