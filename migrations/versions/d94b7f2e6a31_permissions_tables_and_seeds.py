...
def _create_tables(tables: set[str]) -> None:
    """Create the permissions tables (guarded: an existing table is a no-op)."""
...
def _seed_builtin_roles(now: datetime) -> None:
    """Seed the built-in roles (idempotent; a re-run is a no-op)."""
...
def _seed_bootstrap_system_permissions(now: datetime) -> None:
    """Seed the bootstrap system set (idempotent; a re-run is a no-op)."""
...
def upgrade() -> None:
    # Schema (idempotent: an ORM-bootstrapped database is a no-op).
    _create_tables(_existing_tables())
    # Seeds (idempotent: INSERT OR IGNORE; a re-run is a no-op).
    now = datetime.now(UTC)
    _seed_builtin_roles(now)
    _seed_bootstrap_system_permissions(now)
